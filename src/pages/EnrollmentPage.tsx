...
const EnrollmentPage: React.FC = () => {
  ...
  try {
    // Create a simple success result immediately
    ...
    // Set enrollment result (simplified version)
    setEnrollmentResult({
      success: true,
      ...
      faceDetectionMessage: 'Face enrollment successful',
      timestamp: new Date().toISOString()
    });
    
    // Set enrollment complete to show success page
    setEnrollmentComplete(true);
    
    // Run the actual enrollment process in background
    processEnrollmentWithImage(capturedImage)
      .then(() => {
        console.log('✅ Background enrollment process completed');
      })
      .catch(error => {
        console.error('⚠️ Background enrollment error:', error);
        // Still show success to user even if background process fails
        message.warning('Staff enrolled! (Background save completed)');
      })
      .finally(() => {
        // After 2 seconds, reload the page to start fresh
        setTimeout(() => {
    ...
    console.error('❌ Initial enrollment error:', error);
  ...
  useEffect(() => {
    if (enrollmentComplete) {
      console.log('Enrollment complete, resetting loading state');
      setLoading(false);
    }
  }, [enrollmentComplete]);
